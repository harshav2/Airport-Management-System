...
export default function FlightManagement() {
  ...
  const fetchFlights = async () => {
    try {
      setLoading(true);
      const response = await fetch("/api/airline/flight", { method: "GET" });
      if (!response.ok) {
          ...
          <tbody>
            {flights.map((flight) => (
              <tr key={flight.FlightID}>
                ...
                <td className="px-6 py-4 whitespace-nowrap border-b border-gray-300">
                  {/* Render Select only on client-side to avoid hydration issues */}
                  <Select
                    value={flight.Status}
                    onValueChange={(value) => updateFlightStatus(flight.FlightID, value)}
                  >
